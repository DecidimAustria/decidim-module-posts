  ...
  module Feeds
    ...
    class AdminEngine < ::Rails::Engine
      ...
      routes do
        ...
        # The following doesn't work because decidim-feeds is a component and a participatory space
        ...
        # end
        scope "/feeds/:feed_slug/components/:component_id/manage" do
          Decidim.component_manifests.select {|c| c.name == :feeds}.each do |manifest|
            next unless manifest.admin_engine

            constraints CurrentComponent.new(manifest) do
              resources :posts, as: "decidim_admin_feed_#{manifest.name}"
              # mount manifest.admin_engine, at: "/", as: "decidim_admin_feed_#{manifest.name}"
            end
          end
        end
